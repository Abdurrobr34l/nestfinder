...
import { propertyAPI, reviewAPI, bookingAPI } from "../lib/api"
import { useAuth } from "../Hooks/useAuth"
import type { Property, Review } from "../Types/index"
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      ...
      try {
        ...
        const similarRes = await propertyAPI.getAll({
          city: prop.city,
          limit: "4"
        })
        const similar = similarRes.data.data.data.filter(
          (p: Property) => p._id !== prop._id
        ).slice(0, 3)
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { label: "Listed On", value: new Date(property.createdAt).toLocaleDateString() },
                ].map(item => (
                  <div key={item.label}>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
              ...
              <form onSubmit={handleSubmitReview} className="mb-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl">
                ...
                {reviewError && (
                  <div className="text-red-500 text-sm mb-3">{reviewError}</div>
                )}
                <button
                  type="submit"
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 sticky top-20">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full flex items-center justify-center gap-2 bg-green-600 text-white py-3 rounded-xl hover:bg-green-700 transition-colors font-medium mb-3"
              >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-amber-500 text-white py-3 rounded-xl hover:bg-amber-600 transition-colors font-medium disabled:opacity-60"
              >
                <Calendar size={18} />
                {bookingLoading ? "Booking..." : bookingSuccess ? "Booked! ✓" : "Book a Tour"}
              </button>

              {bookingError && (
                <p className="text-red-500 text-xs mt-2 text-center">{bookingError}</p>
              )}
